...
CUserSession::CUserSession(void)
{
    ...
    connect(m_timer, &QTimer::timeout, this, &CUserSession::slotLogout);
}
...
//SLOT
void CUserSession::slotLogout()
{
    qDebug() << "LOGOUT";
    m_nLastUserType = m_nUserType;
    m_nUserType = GUEST;
    m_timer->stop();
    emit signalUserSessionClosed();
}

//SLOT
void CUserSession::slotPoke()
{
    if(!amIGuest())
    {
        m_timer->start();
...
//void CUserSession::objectRegister(IUserSession* pObject)
//{
...
//void CUserSession::objectUnregister(IUserSession* pObject)
//{
