...
void CComJBus::generateDocumentation(){
    QString dirPath = QString(DOC_DIRECTORY);
    QDir dir = QDir(dirPath);
    if(!dir.exists()){
        dir.mkdir(dirPath);
    }
    QString path = dirPath+ "/JBusTable_"+ m_name +".tex";
    QFile data(path);
    if(data.exists())
        data.remove();
    if (data.open(QFile::Append)) {
        ...
        out << "\\title{"<< QString(tr("JBUS Datas tables")) << "}"<< endl;
        out << "\\author{" << QString(tr("Generated by SERES TOC")) << "}"<< endl;
        out << "\\begin{document}"<< endl;
        out << "\\maketitle"<< endl;

        //Bits data table
        out << "\\begin{table}[!h] \\centering" << endl;
        out << "\\caption{" <<QString(tr("Input/Ouput bits data table")) << "}" << endl;
        out << "\\begin{tabular}{|l|l|l|}"<< endl;
        out << "\\hline"<< endl;
        out << "\\textbf{Data Name}  & \\textbf{Address} \\\\"<< endl;
        out << "\\hline"<< endl;
        ...
        //Words data table
        out << "\\begin{table}[!h] \\centering" << endl;
        out << "\\caption{"  << QString(tr("Input/Ouput words data table")) << "}" << endl;
        out << "\\begin{tabular}{|l|l|l|}"<< endl;
        out << "\\hline"<< endl;
